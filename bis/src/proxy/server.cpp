...
int Server::Init() {
    ...
    do {
        if ((ret = LPEPOLLHLD->EpollInit()) < 0)
            break;
        if ((ret = LPMQMGR->Init() ) < 0)
            break;
    } while (false);
...
void Server::RemoveTask(const TaskMapItr &itr) {
...
bool Server::OnMqRecv() {
    //LOG(INFO) << ">>>>trace>>>>>" << endl;
    vector<string> msgs;

    if (LPMQMGR->RecvMsgs(msgs) <= 0)
        return false;

    for (unsigned int i = 0; i < msgs.size(); ++i) {
        bs_czl::MsgTransApp msg;
        if (!msg.ParseFromString(msgs[i])) {
            LOG(WARNING) << "Parse String to MsgTransApp Fail.("
                         << msgs[i].size()
                         << ")" << endl;
            continue;
        }
        LOG(INFO) << msg.ShortDebugString() << endl;
        TaskPtr task = FindTask(static_cast<unsigned int>(msg.client_pos()));
        if (!task) {
            LOG(WARNING) << "Can't Find Task." << msg.client_pos() << endl;
            continue;
        }
        task->OnMQRecv(msg);
    }
