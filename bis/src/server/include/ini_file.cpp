        ...
        const char *szDefault, char *szValue, const int iValueLen)
{
    ...
    while (pszTemp < m_szBuffer + m_iSize)
    {
        ...
        if (*(pszItem + iItemLen) != ' ' && *(pszItem + iItemLen) != '=' && *(pszItem
                + iItemLen) != '\t')
        {
            pszTemp = pszItem + iItemLen;
            continue;
...
int CIniFile::GetString(const char *sSection, const char *sItem,
              const char *sDefault, std::string &strValue)
{
    char buffer[2048] = {0};
    int ret = 0;
    ret = GetString(sSection, sItem, sDefault, buffer, 2048);
