...
/**
 * ChangeLog
 * 20100401 jamieli 对碎片整理的BUG进行修复
 * 20100402 jamieli 修改预定义HDBXFSIZINC，每次文件增加8M
 */

#ifdef JMDEBUG
#define TCDBG(args...) printf(##args)
#else
#define TCDBG(args...)
#endif
...
#define HDBXFSIZINC    (1<<20)           // increment of extra file size
...
typedef struct
{ // type of structure for a duplication callback
    TCPDPROC proc; // function pointer
    void *op; // opaque pointer
} HDBPDPROCOP;

/* private macros */
//锁数据库头部信息（DB_HEADER BUTTER）
...
static bool tchdbputimpl(TCHDB *hdb, const char *kbuf, int ksiz, uint64_t bidx, uint8_t hash, const char *vbuf, int vsiz, int dmode);
static void tchdbdrpappend(TCHDB *hdb, const char *kbuf, int ksiz, const char *vbuf, int vsiz, uint8_t hash);
static bool tchdbputasyncimpl(TCHDB *hdb, const char *kbuf, int ksiz, uint64_t bidx, uint8_t hash, const char *vbuf, int vsiz);
static bool tchdboutimpl(TCHDB *hdb, const char *kbuf, int ksiz, uint64_t bidx, uint8_t hash);
static char *tchdbgetimpl(TCHDB *hdb, const char *kbuf, int ksiz, uint64_t bidx, uint8_t hash, int *sp);
static int tchdbgetintobuf(TCHDB *hdb, const char *kbuf, int ksiz, uint64_t bidx, uint8_t hash, char *vbuf, int max);
...
{
    assert(hdb);
    return hdb->mmtx ? (int) (intptr_t) pthread_getspecific(*(pthread_key_t *) hdb->eckey) : hdb->ecode;
}
...
bool tchdbsetmutex(TCHDB *hdb)
{
    ...
    if (pthread_mutexattr_settype(&rma, PTHREAD_MUTEX_RECURSIVE) != 0)
        err = true;
    if (pthread_rwlock_init(hdb->mmtx, NULL) != 0)
        err = true;
    for (int i = 0; i <= UINT8_MAX; i++)
    {
        if (pthread_rwlock_init((pthread_rwlock_t *) hdb->rmtxs + i, NULL) != 0)
            err = true;
    }
    if (pthread_mutex_init(hdb->dmtx, &rma) != 0)
        err = true;
    if (pthread_mutex_init(hdb->wmtx, NULL) != 0)
        err = true;
    if (pthread_key_create(hdb->eckey, NULL) != 0)
        err = true;
    if (err)
    {
...
{
    TCDBG("%s|%d|tchdbput ksiz=%d vsiz=%d\n", __func__, __LINE__, ksiz, vsiz);
    ...
    if (hdb->zmode)
    {
        ...
        HDBUNLOCKRECORD(hdb, bidx);
        HDBUNLOCKMETHOD(hdb);
        if (hdb->dfunit > 0 && hdb->dfcnt > hdb->dfunit && !tchdbdefrag(hdb, hdb->dfunit * HDBDFRSRAT + 1))
            rv = false;
        return rv;
    }
...
void tchdbsetecode(TCHDB *hdb, int ecode, const char *filename, int line, const char *func)
{
    ...
    if (ecode != TCESUCCESS && ecode != TCEINVALID && ecode != TCEKEEP && ecode != TCENOREC && ecode != TCEDBFULL)
    {
        hdb->fatal = true;
        if (hdb->fd >= 0 && (hdb->omode & HDBOWRITER))
            tchdbsetflag(hdb, HDBFFATAL, true);
    }
    if (hdb->dbgfd >= 0 && (hdb->dbgfd != UINT16_MAX || hdb->fatal))
    {
        int dbgfd = (hdb->dbgfd == UINT16_MAX) ? 1 : hdb->dbgfd;
        char obuf[HDBIOBUFSIZ];
        int osiz = sprintf(obuf, "ERROR:%s:%d:%s:%s:%d:%s:%d:%s\n", filename, line, func, hdb->path ? hdb->path : "-", ecode, tchdberrmsg(ecode), myerrno, strerror(myerrno));
...
bool tchdbdefrag(TCHDB *hdb, int64_t step)
{
    ...
    while (!err && !stop)
    {
        if (HDBLOCKALLRECORDS(hdb, true))
        {
            ...
            if (!tchdbdefragimpl(hdb, UINT8_MAX))
                err = true;
            if (hdb->dfcur <= cur)
                stop = true;
            HDBUNLOCKALLRECORDS(hdb);
            HDBTHREADYIELD(hdb); //在多线程情况下，这里应该是将CPU时间片让别人用吧？？
        }
        else
        {
            err = true;
...
//将数据(buf/size)写入到数据文件hdb->fd的off位置
static bool tchdbseekwrite(TCHDB *hdb, off_t off, const void *buf, size_t size)
{
    assert(hdb && off >= 0 && buf && size >= 0);
    if (hdb->tran && !tchdbwalwrite(hdb, off, size))
        return false;
    off_t end = off + size;
    if (end <= hdb->xmsiz)
    {
...
/* Seek and read data from a file.
 ...
 `size' specifies the size of the buffer.
 The return value is true if successful, else, it is false. */
//从数据文件hdb->fd的off位置，读取size个字节，写入到buf中
static bool tchdbseekread(TCHDB *hdb, off_t off, void *buf, size_t size)
{
    assert(hdb && off >= 0 && buf && size >= 0);
    ...
    if (!TCUBCACHE && off < hdb->xmsiz)
    {
        //如果系统支持UBCACHE，直接从文件里面读取，否则先从MMAP中读取一部分，再从文件中读取
        int head = hdb->xmsiz - off;
...
static bool tchdbseekreadtry(TCHDB *hdb, off_t off, void *buf, size_t size)
{
    ...
    //下面的情况就是一部分数据在MMAP范围内，一部分在MMAP范围外
    if (!TCUBCACHE && off < hdb->xmsiz)
    {
        int head = hdb->xmsiz - off;
        memcpy(buf, hdb->map + off, head);
...
static uint64_t tchdbbidx(TCHDB *hdb, const char *kbuf, int ksiz, uint8_t *hp)
{
    //TODO 这里的HASH算法可以根据具体业务逻辑（例如UIN）特性优化，以提高性能
...
static void tchdbsetbucket(TCHDB *hdb, uint64_t bidx, uint64_t off)
{
    ...
    if (hdb->ba64)
    {
        ...
        if (hdb->tran)
            tchdbwalwrite(hdb, HDBHEADSIZ + bidx * sizeof(llnum), sizeof(llnum));
    ...
    else
    {
        ...
        if (hdb->tran)
            tchdbwalwrite(hdb, HDBHEADSIZ + bidx * sizeof(lnum), sizeof(lnum));
        hdb->ba32[bidx] = TCHTOIL(lnum);
    }
}

/* Save the free block pool to the file.
 The return value is true if successful, else, it is false. */
static bool tchdbsavefbp(TCHDB *hdb)
{
    assert(hdb);
...
/* Load the free block pool into the file.
 The return value is true if successful, else, it is false. */
static bool tchdbloadfbp(TCHDB *hdb)
{
    int bsiz = hdb->frec - hdb->msiz;
...
static void tcfbpsortbyoff(HDBFB *fbpool, int fbpnum)
{
    ...
    while (bottom > 0)
    {
        ...
        while (i <= top)
        {
            if (i < top && fbpool[i + 1].off > fbpool[i].off)
                i++;
...
static void tchdbfbpmerge(TCHDB *hdb)
{
    ...
    while (cur < end)
    {
        if (cur->off > 0)
        {
            ...
            {
                TCDBG("FBM|%llu|%d|%d|%d\n", cur->off, cur->rsiz, next->rsiz, cur->rsiz+next->rsiz);
...
static bool tchdbfbpsearch(TCHDB *hdb, TCHREC *rec)
{
    TCDBG("%s|%d tchdbfbpsearch size=%d\n", __func__, __LINE__, rec->rsiz);

    assert(hdb && rec);
    TCDODEBUG(hdb->cnt_searchfbp++);
    if (hdb->fbpnum < 1)
    {
        //如果没有FB，直接蒋rec的off指向文件末尾，然后标记现有空间为0
        rec->off = hdb->fsiz;
        rec->rsiz = 0;
        return true;
    }
    ...
    //二分法搜索大于数据块要求的FB，如果搜索到的FB大于两倍要求的空间，空闲部分将作为新的FB插入到HDB
    int i = (left + right) / 2;
    int cand = -1;
    while (right >= left && i < num)
    {
    ...
    if (cand >= 0)
    {
        ...
        if (pv->rsiz > rsiz * 2)
        {
            //如果pv对应的rsize是当前记录的2倍以上，需要分裂该fb
            uint32_t psiz = tchdbpadsize(hdb, pv->off + rsiz);
            uint64_t noff = pv->off + rsiz + psiz;  //新fb的offset
            if (pv->rsiz >= (noff - pv->off) * 2)
            {
                ...
                pv->off = noff;
                pv->rsiz -= rec->rsiz;
                //BUGFIX 这里fb写入以后，没有对fb进行重新排序，但是从搜索的逻辑来看，也关系不大
                return tchdbwritefb(hdb, pv->off, pv->rsiz);
            }
        }

        TCDBG("FBD|%d|%llu|%d|%d\n", rec->rsiz, pv->off, pv->rsiz, hdb->fbpnum);
    ...
    //没有搜索到合适的fb，考虑在文件末尾进行扩
    rec->off = hdb->fsiz;
    rec->rsiz = 0;
    hdb->fbpmis++;
    TCDBG("fbmiss=%d\n", hdb->fbpmis);

    //每超过4096次fbpmiss，就会进行一次碎片整理
...
//查看rec对应的节点后面是否有fb，如果有，尽量合并到nsiz大小
static bool tchdbfbpsplice(TCHDB *hdb, TCHREC *rec, uint32_t nsiz)
{
    ...
    {
        //先检查紧挨着后面的FB是否够存放
        //如果设置了mmtx，只检查自己后面紧挨着的FB
        if (hdb->fbpnum < 1)
            return false;
        uint64_t off = rec->off + rec->rsiz;
        uint32_t rsiz = rec->rsiz;
        uint8_t magic;
        if (tchdbseekreadtry(hdb, off, &magic, sizeof(magic)) && magic != HDBMAGICFB)
            return false;
        HDBFB *pv = hdb->fbpool;
    ...
    {
        nrec.off = off;
        if (!tchdbreadrec(hdb, &nrec, nbuf))
            return false;
        if (nrec.magic != HDBMAGICFB)
            break;
        if (hdb->dfcur == off)
            hdb->dfcur += nrec.rsiz;
        if (hdb->iter == off)
...
/* Remove blocks of a region from the free block pool.
 ...
 `off' specifies the offset of the block.
 `rsiz' specifies the size of the block. */
//将FB列表中base与next之间的FB删掉，并且将off/rsiz对应的FB加入到FB列表中
static void tchdbfbptrim(TCHDB *hdb, uint64_t base, uint64_t next, uint64_t off, uint32_t rsiz)
{
...
static bool tchdbwritefb(TCHDB *hdb, uint64_t off, uint32_t rsiz)
{
    ...
    *(uint8_t *) (wp++) = HDBMAGICFB;
    ...
    wp += sizeof(lnum);
    if (!tchdbseekwrite(hdb, off, rbuf, wp - rbuf))
        return false;
    return true;
}
...
/* Write a record into the file.
 ...
 The return value is true if successful, else, it is false. */
//将rec写入数据文件中，entoff表示父指针的偏移
static bool tchdbwriterec(TCHDB *hdb, TCHREC *rec, uint64_t bidx, off_t entoff)
{
    TCDBG("%s|%d|tchdbfbpsearch off=%d, rsize=%d, entoff=%d\n", __func__, __LINE__, rec->off, rec->rsiz, entoff);
    assert(hdb && rec);
    TCDODEBUG(hdb->cnt_writerec++);
    char stack[HDBIOBUFSIZ];

    //计算出记录需要占用的空间（buffer size），如果rec->rsiz大于0，表示已经计算出空间了，否则重新计算空间
    //rec->rsiz只是记录了被rec覆盖的那个节点在数据文件中存在的空间，该空间要么肯定大于rec header的长度，要么等于0（表示没有找打对应的记录覆盖，需要在文件后面扩数据的）
    int bsiz = (rec->rsiz > 0) ? rec->rsiz : HDBMAXHSIZ + rec->ksiz + rec->vsiz + hdb->align;
    char *rbuf; //record buff
    if (bsiz <= HDBIOBUFSIZ)
    {
        rbuf = stack;
    }
    ...
    *(uint8_t *) (wp++) = rec->hash;
    ...
    uint16_t snum;
    char *pwp = wp; //padding size对应的位置
    wp += sizeof(snum);
    int step;
    TCSETVNUMBUF(step, wp, rec->ksiz);  //KEY长度7BIT编码
    wp += step;
    TCSETVNUMBUF(step, wp, rec->vsiz);  //VAL长度7BIT编码
    wp += step;
    int32_t hsiz = wp - rbuf; //header size
    int32_t rsiz = hsiz + rec->ksiz + rec->vsiz; //record size(data in rec object)，不包含PADDING部分
    int32_t finc = 0; //file increase size
    if (rec->rsiz < 1)
    {
    ...
    else if (rsiz > rec->rsiz)
    {
        ...
        //如果rec后面不是FB，或者后面连续的FB加起来空间还不够，将自身写为FB回收
    ...
    if (entoff > 0)
    {
        if (hdb->ba64)
        {
            ...
            llnum = TCHTOILL(llnum);
            if (!tchdbseekwrite(hdb, entoff, &llnum, sizeof(uint64_t)))
                return false;
        }
        else
        {
            ...
            lnum = TCHTOIL(lnum);
            if (!tchdbseekwrite(hdb, entoff, &lnum, sizeof(uint32_t)))
                return false;
        }
    }
    else
...
/* Read a record from the file.
 ...
 The return value is true if successful, else, it is false. */
//从rec->off的位置，读取一个hdb->runit单位的数据，主要是读取包头部分的信息
//将recored header的数据解开，写入rec对应的地址，如果读取的runit包含了key和val的值，keybuff和valbuff指针将被赋值，否则为NULL
//函数内部会根据rec的类型计算出准确的rsize
//TODO hdb->runit的调整可以影响性能
static bool tchdbreadrec(TCHDB *hdb, TCHREC *rec, char *rbuf)
{
    assert(hdb && rec && rbuf);
...
/* Read the body of a record from the file.
 ...
 `rec' specifies the record object.
 The return value is true if successful, else, it is false. */
//读取记录的key与val内容部分，分配keysiz+valsiz+1长度的空间，需要函数外部释放
static bool tchdbreadrecbody(TCHDB *hdb, TCHREC *rec)
{
    assert(hdb && rec);
...
static bool tchdbremoverec(TCHDB *hdb, TCHREC *rec, char *rbuf, uint64_t bidx, off_t entoff)
{
    ...
    else
    {
        ...
        if (hdb->ba64)
        {
            ...
            llnum = TCHTOILL(llnum);
            if (!tchdbseekwrite(hdb, toff, &llnum, sizeof(uint64_t)))
                return false;
        }
        else
        {
            ...
            lnum = TCHTOIL(lnum);
            if (!tchdbseekwrite(hdb, toff, &lnum, sizeof(uint32_t)))
                return false;
        }
    }
    if (entoff > 0)
...
/* Remove a record from the file.
 ...
 The return value is true if successful, else, it is false. */
//将rec所对应的记录，写到destoff对应的位置，如果destoff对应的位置没有空间，destoff对应位置对应的rec将变为fb，节点将被写到其他地方去
static bool tchdbshiftrec(TCHDB *hdb, TCHREC *rec, char *rbuf, off_t destoff)
{
    ...
    if (rec->off == off)
    {
        bool err = false;
        rec->off = destoff;
        if (!tchdbwriterec(hdb, rec, bidx, 0))
            err = true;
        TCFREE(rec->bbuf);
        rec->kbuf = NULL;
        rec->vbuf = NULL;
...
static int tchdbwalrestore(TCHDB *hdb, const char *path)
{
    ...
    if (walsiz >= sizeof(walsiz) + HDBHEADSIZ)
    {
        ...
        size_t xmsiz = 0;
        if (hdb->fd >= 0 && hdb->map)
            xmsiz = (hdb->xmsiz > hdb->msiz) ? hdb->xmsiz : hdb->msiz;
...
static bool tchdbopenimpl(TCHDB *hdb, const char *path, int omode)
{
    ...
    if (!(omode & HDBONOLCK))
    {
        if (memcmp(hbuf, HDBMAGICDATA, strlen(HDBMAGICDATA)) || hdb->type != type || hdb->frec < msiz + HDBFBPBSIZ || hdb->frec > hdb->fsiz || sbuf.st_size < hdb->fsiz)
        {
    ...
    if (((hdb->opts & HDBTDEFLATE) && !_tc_deflate) || ((hdb->opts & HDBTBZIP) && !_tc_bzcompress) || ((hdb->opts & HDBTEXCODEC) && !hdb->enc))
    {
        tchdbsetecode(hdb, TCEINVALID, __FILE__, __LINE__, __func__);
        close(fd);
        return false;
    ...
    if (!(omode & HDBOWRITER) && xmsiz > hdb->fsiz)
        xmsiz = hdb->fsiz;
    void *map = mmap(0, xmsiz, PROT_READ | ((omode & HDBOWRITER) ? PROT_WRITE : 0), MAP_SHARED, fd, 0);
    if (map == MAP_FAILED)
    {
        tchdbsetecode(hdb, TCEMMAP, __FILE__, __LINE__, __func__);
        close(fd);
    ...
    hdb->zmode = (hdb->opts & HDBTDEFLATE) || (hdb->opts & HDBTBZIP) || (hdb->opts & HDBTTCBS) || (hdb->opts & HDBTEXCODEC);
    hdb->ecode = TCESUCCESS;
    hdb->fatal = false;
    ...
    {
        bool err = false;
        if (!(hdb->flags & HDBFOPEN) && !tchdbloadfbp(hdb))
            err = true;
        //如果FB信息被加载出来，文件里面的FB信息将被清空
        memset(hbuf, 0, 2);
        if (!tchdbseekwrite(hdb, hdb->msiz, hbuf, 2))
            err = true;
        if (err)
        {
            TCFREE(hdb->path);
...
static bool tchdbcloseimpl(TCHDB *hdb)
{
    ...
    if ((hdb->omode & HDBOWRITER) && !tchdbmemsync(hdb, false))
        err = true;
    size_t xmsiz = (hdb->xmsiz > hdb->msiz) ? hdb->xmsiz : hdb->msiz;
    if (!(hdb->omode & HDBOWRITER) && xmsiz > hdb->fsiz)
        xmsiz = hdb->fsiz;
...
static bool tchdbputimpl(TCHDB *hdb, const char *kbuf, int ksiz, uint64_t bidx, uint8_t hash, const char *vbuf, int vsiz, int dmode)
{
    assert(hdb && kbuf && ksiz >= 0);
    if (hdb->recc)
        tcmdbout(hdb->recc, kbuf, ksiz);
    off_t off = tchdbgetbucket(hdb, bidx);
    off_t entoff = 0; //父节点记录该节点的便宜量
    TCHREC rec;
    ...
    while (off > 0)
    {
        ...
        {
            off = rec.right;
            entoff = rec.off + (sizeof(uint8_t) + sizeof(uint8_t)) + (hdb->ba64 ? sizeof(uint64_t) : sizeof(uint32_t));
        }
        else
        {
            //如果hash值相同，第二层比较key的值
            ...
            else
            {
                ...
                switch (dmode)
                {
                    ...
                    case HDBPDCAT:
                        ...
                        //根据需要嫁接的数据，重新组织rec的内容
                        nvsiz = rec.vsiz + vsiz;    //new val size
                        if (rec.bbuf)
                        {
    ...
    rec.rsiz = hdb->ba64 ? sizeof(uint8_t) * 2 + sizeof(uint64_t) * 2 + sizeof(uint16_t) : sizeof(uint8_t) * 2 + sizeof(uint32_t) * 2 + sizeof(uint16_t);
    //由于keysize和valsize编码的时候是按照7bit编码的，所以在这里要进行判断
    if (ksiz < (1U << 7))
    {
        rec.rsiz += 1;
    }
...
static bool tchdbputasyncimpl(TCHDB *hdb, const char *kbuf, int ksiz, uint64_t bidx, uint8_t hash, const char *vbuf, int vsiz)
{
    ...
    while (off > 0)
    {
        ...
        { //??
            ...
            if (TCXSTRSIZE(hdb->drpdef) > HDBDRPUNIT && !tchdbflushdrp(hdb))
                return false;
            return true;
        }
        rec.off = off;
    ...
    hdb->rnum++;
    if (TCXSTRSIZE(hdb->drpool) > HDBDRPUNIT && !tchdbflushdrp(hdb))
        return false;
    return true;
}
        ...
        else
        {
            ...
            else
            {
                ...
                if (hdb->zmode)
                {
                    ...
                    if (hdb->recc)
                    {
                        if (tcmdbrnum(hdb->recc) >= hdb->rcnum)
...
static bool tchdbiternextintoxstr(TCHDB *hdb, TCXSTR *kxstr, TCXSTR *vxstr)
{
    ...
    while (hdb->iter < hdb->fsiz)
    {
        ...
        if (!tchdbreadrec(hdb, &rec, rbuf))
            return false;
        hdb->iter += rec.rsiz;
        TCDBG("REC|%llu|%d|0x%x\n", rec.off, rec.rsiz, rec.magic);
        if (rec.magic == HDBMAGICREC)
        {
            if (!rec.vbuf && !tchdbreadrecbody(hdb, &rec))
                return false;
...
static bool tchdbcopyimpl(TCHDB *hdb, const char *path)
{
    ...
    if (*path == '@')
    {
        ...
        args[1] = hdb->path;
        args[2] = tsbuf;
        if (tcsystem(args, sizeof(args) / sizeof(*args)) != 0)
            err = true;
    }
    else
    {
...
static bool tchdbdefragimpl(TCHDB *hdb, int64_t step)
{
    ...
    while (true)
    {
        ...
        if (step-- < 1)
            return true; //本次碎片整理最多向后扫描step个记录
        rec.off = hdb->dfcur;
        if (!tchdbreadrec(hdb, &rec, rbuf))
            return false;
        if (rec.magic == HDBMAGICFB)
            break;
        //在step中只要扫描到一个FB，退出扫描，进行后面的处理
        hdb->dfcur += rec.rsiz;
    }
    ...
    if (cur < hdb->fsiz)
    {
        ...
        {
            //如果FB超大，将FB分成多个插入到FBLIST中
            tchdbfbptrim(hdb, dest, cur, 0, 0);
            uint64_t off = dest;
            uint64_t size = fbsiz;
            while (size > 0)
            {
                uint32_t rsiz = (size > HDBFBMAXSIZ) ? HDBFBMAXSIZ : size;
                if (size - rsiz < HDBMINRUNIT)
                    rsiz = size;
                tchdbfbpinsert(hdb, off, rsiz);
                if (!tchdbwritefb(hdb, off, rsiz))
                    return false;
                off += rsiz;
                size -= rsiz;
            }
        ...
        {
            //删除base到cur之间的FB，然后插入dest/fbsiz这个FB
            //BUGFIX:由于base指向的位置，可能出现新的FB，这个FB不在dest/fbsize的包含范围内，这种FB不能被删除
            //这里只能删除从dest到cur之间的FB
            tchdbfbptrim(hdb, dest, cur, dest, fbsiz);
            if (!tchdbwritefb(hdb, dest, fbsiz))
                return false;
        }
        hdb->dfcur = cur - fbsiz;
    }
...
{
    assert(hdb);
    if (wr ? pthread_rwlock_wrlock((pthread_rwlock_t *) hdb->rmtxs + bidx) != 0 : pthread_rwlock_rdlock((pthread_rwlock_t *) hdb->rmtxs + bidx) != 0)
    {
        tchdbsetecode(hdb, TCETHREAD, __FILE__, __LINE__, __func__);
        return false;
    }
...
static bool tchdblockallrecords(TCHDB *hdb, bool wr)
{
    ...
    for (int i = 0; i <= UINT8_MAX; i++)
    {
        if (wr ? pthread_rwlock_wrlock((pthread_rwlock_t *) hdb->rmtxs + i) != 0 : pthread_rwlock_rdlock((pthread_rwlock_t *) hdb->rmtxs + i) != 0)
        {
            tchdbsetecode(hdb, TCETHREAD, __FILE__, __LINE__, __func__);
            while (--i >= 0)
            {
...
static bool tchdbunlockallrecords(TCHDB *hdb)
{
    ...
    for (int i = UINT8_MAX; i >= 0; i--)
    {
        if (pthread_rwlock_unlock((pthread_rwlock_t *) hdb->rmtxs + i))
            err = true;
    }
    TCTESTYIELD();
    if (err)
